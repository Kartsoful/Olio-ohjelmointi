...
void HttpPerson::personDataSlot(QNetworkReply *reply)
{
    ...
    // Käsitellään JSON-arrayta
    for (const QJsonValue &value : json_array) {
        if (value.isObject()) {
            qDebug() << value["firstname"].toString() << value["lastname"].toString();
        }
    }
...
void HttpPerson::onePersonDataSlot(QNetworkReply *reply)
{
    ...
    // Muunnetaan vastaus QByteArray-tyyppisestä JSON-dokumentiksi
    QJsonDocument json_doc = QJsonDocument::fromJson(response_data);
    // Muutetaan JSON-dokumentti JSON-objectiksi
    QJsonObject json_obj = json_doc.object();
    //Tulostetaan henkilön etu- ja sukunimi
